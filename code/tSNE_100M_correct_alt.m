...
CombinedMatrix_S7_100M_alt(1006:1105,1:1005) = dataSample_S7_100M_country';
CombinedMatrix_S7_100M_alt(1:1005,1006:1105) = dataSample_S7_100M_country;
for i = 1:1105
    for j = i+1:1105
...
CombinedMatrix_100M = CombinedMatrix_S7_100M_alt(1:1005, 1:1005);

% tsne plot
T_sne_100M = tsne(CombinedMatrix_100M,'Algorithm','exact','Distance','cosine');
% % extract data (Top Followers)
tSNE_100M_alt = T_sne_100M(1:1005,:);
...
writetable(tSNE_table_100M,'tSNE_table_100M.csv','Encoding','UTF-8');
...
figure('units','inch','position',[20,12,20,12]); % this is a good fit size for the window
gscatter(tSNE_table_100M.C1,tSNE_table_100M.C2, ...
    tSNE_table_100M.Category, 'rkgbcm');
labelpoints(tSNE_table_100M.C1,tSNE_table_100M.C2,tSNE_table_100M.screen_name, ...
...
h = [p1(1); p2; p3(1); p4; p5(1); p6;p7(1);p8;p9(1);p10; ... 
    p11(1);p12;p13(1);p14;p15(1);p16;p17(1);p18;p19(1);p20];
title('\rm 100 Millions Sampled Users from Top Followers in Twitter', 'FontSize', 18);
labelpoints(tSNE_table_100M.C1,tSNE_table_100M.C2,tSNE_table_100M.screen_name, ...
'C',0.025, 1, 'FontSize', 1.25,  'rotation', 0,  'Color', 'k'); % location label 'SE' % note: C represents much better
...
grid off;
hold off;
print(gcf,'tSNE_Language_TopFollowers.png','-dpng','-r1300');
